...
void oled_power_save(void) {
    ...
    u8g2_SetPowerSave(&u8g2, 1);  // Note: 1 = sleep, 0 = wake
}
...
//     // Toggle rotation
//     current_rotation = (current_rotation == U8G2_R0) ? U8G2_R2 : U8G2_R0;

//     u8g2_SetDisplayRotation(&u8g2, current_rotation);
//     i2c_task_send_display_text("Rotated!");
// }
...
esp_err_t oled_display_text(const char* text) {
    ...
    const int line_height = 10;  // Adjusted for profont10
    const int max_width = 128;   // Display width in pixels
    const int max_lines = 6;     // Maximum number of lines for 64px height (or 3 for 32px height)
    
    int y = 10;  // Starting y position (adjusted for font baseline)
    int line_count = 0;
