...
static bool toggle_reset = false;
...
static void button_s1_single_callback(void *arg) {
    ...
    if (spam_timer_active) {
        // printf("S1 single press ignored\n");
...
static void button_s1_medium_callback(void *arg) {
    ...
    if (spam_timer_active) {
        // printf("S1 medium press ignored\n");
...
static void button_s1_long_callback(void *arg) {
    ...
    // Toggle between 5 seconds and 30 seconds
    off_extended_delay = !off_extended_delay;

    TickType_t new_period = off_extended_delay ? pdMS_TO_TICKS(30000) : pdMS_TO_TICKS(5000);

    printf("OLED disconnect delay set to %d seconds.\n", off_extended_delay ? 30 : 5);
...
static void button_s1_double_callback(void *arg) {
    ...
    if (spam_timer_active) {
        // printf("S1 double click ignored\n");
...
static void button_s2_single_callback(void *arg) {
    ...
    if (spam_timer_active) {
        // printf("S2 single press ignored\n");
...
static void button_s2_medium_callback(void *arg) {
    ...
    if (spam_timer_active) {
        // printf("S2 medium press ignored\n");
...
static void button_s2_double_callback(void *arg) {
    ...
    if (spam_timer_active) {
        // printf("S2 double click ignored\n");
...
esp_err_t button_manager_init(gpio_num_t button_pin_s1, gpio_num_t button_pin_s2) {
    ...
    oled_timer = xTimerCreate("OLED Timer", pdMS_TO_TICKS(15000), pdFALSE, NULL, oled_disconnect_callback);
    if (oled_timer == NULL) {
        printf("Failed to create OLED timer\n");
        return ESP_FAIL;
    }

    // Create debounce timers for each button
    spam_timer = xTimerCreate("Spam Timer", pdMS_TO_TICKS(200), pdFALSE, NULL, spam_timer_callback);
    if (spam_timer == NULL) {
