...
void clear_top_requests() {
    ...
    ESP_LOGW(SNIFFER_TAG, "Top requests cleared");
}
...
void display_top_requests_oled(lv_disp_t *disp) {
    ...
    // Define and assign 'a' based on the condition
    char disp_delay[8];
    // if (off_extended_delay) {
    //     snprintf(disp_delay, sizeof(disp_delay), "30s");
    // } else {
    //     snprintf(disp_delay, sizeof(disp_delay), "5s");
    // }
    ...
    if (index_to_display >= 0 && index_to_display < max_request_rank) {
        if (top_requests[index_to_display].rssi != -101) {  // Check if it's a valid entry
            ...
            snprintf(display_text, sizeof(display_text),
                     "(%d/%d) Top RSSI    [%s]\nRSSI: %d\nTime: %s\n%s", 
                     request_index, max_request_rank, disp_delay, top_requests[index_to_display].rssi, time_buf, top_requests[index_to_display].mac_address);
...
static void sniffer_task(void *parameters)
{
    ...
    while (sniffer->is_running)
    {
        ...
        if (xTaskGetTickCount() - last_update_time >= pdMS_TO_TICKS(UPDATE_INTERVAL_MS))
        {
            ...
            clear_counter++;
            // Only clear top requests every 10th time
            if (clear_counter >= 10) {
                clear_top_requests();
                clear_counter = 0;  // Reset the counter
            }
