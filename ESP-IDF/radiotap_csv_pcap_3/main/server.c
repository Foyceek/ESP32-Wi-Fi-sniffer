...
esp_err_t create_zip_archive(const char *directory_path, const char *zip_path) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_type == DT_REG) {
            // Only process files with .pcap extension
            if (!strstr(entry->d_name, ".pcap")) {
                ESP_LOGW(TAG, "Skipping non-pcap file: %s", entry->d_name);
                continue;
            }
...
esp_err_t browse_sd_get_handler(httpd_req_t *req) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        size_t needed_size = strlen(response) + 512;  // Estimate size for each file entry
        if (needed_size >= response_size) {
            response_size *= 2;  // Double the buffer size
            response = realloc(response, response_size);
            if (!response) {
                ESP_LOGE(TAG, "Failed to reallocate memory for SD card response.");
                httpd_resp_send_500(req);
                closedir(dir);
                return ESP_FAIL;
            }
    ...
    strncat(response, "</ul></body></html>", response_size - strlen(response) - 1);
