...
esp_err_t pcap_print_summary(pcap_file_handle_t pcap, FILE *print_file)
{
    ...
    while (index < size) {
        ...
        fprintf(print_file, "Packet %lu:\n", packet_num);
        fprintf(print_file, "Timestamp (Seconds): %lu\n", packet_header.seconds);
        fprintf(print_file, "Timestamp (Microseconds): %lu\n", packet_header.microseconds);
        fprintf(print_file, "Capture Length: %lu\n", packet_header.capture_length);
        fprintf(print_file, "Packet Length: %lu\n", packet_header.packet_length);
        ...
        if (file_header.link_type == PCAP_LINK_TYPE_802_11) {
            ...
            fprintf(print_file, "Unknown link type:%lu\n", file_header.link_type);
            fprintf(print_file, "------------------------------------------------------------------------\n");
        }
        free(packet_payload);
        packet_payload = NULL;
    ...
    fprintf(print_file, "Pcap packet Number: %lu\n", packet_num);
    fprintf(print_file, "------------------------------------------------------------------------\n");
    return ret;
err:
    if (packet_payload) {
