...
df = pd.read_csv('utery_final.csv', encoding='utf-8')

# Read packet data
packet_df = pd.read_csv('anonymized_relevant_data_utery.csv')
...
STOP_ORDER = [
    ...
    "Tomkovo náměstí",
    "Maloměřický most",
    "Maloměřický most - výstup",
    "Maloměřický most - nástup",
    "Proškovo náměstí",
    "Obřanská - u školy",
    "Čtvery hony",
    "Obřanský most",
    "Obřany, Babická - nástup",
    "Obřany, Babická - výstup",
]
...
def plot_segments_unified_heatmap(segments, packet_df):
    ...
    # Add title
    ax.set_title("Úterý, Linka 4 - Hustota zachycených probe requestů", 
    ...
    plt.subplots_adjust(bottom=0.27, top=0.93, left=0.05, right=0.98)
